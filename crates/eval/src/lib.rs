...
impl<'runtime> Evaluator<'runtime> {
    ...
    fn evaluate_list_schema_call(
        ...
        let EvaluatedValue::List(items) = resolve_default_value(list_value) else {
            return Ok(invalid_list_builtin_arg(name));
        };
    ...
    fn evaluate_sort_direction(
        ...
        match resolve_default_value(self.evaluate_expr_at(comparator, environment, depth + 1)?) {
    ...
    fn evaluate_comprehension_clause(
        ...
        match clause {
            ComprehensionClause::If { condition } => {
                match self.evaluate_expr_at(*condition, environment, depth + 1)? {
...
fn comprehension_items(
    ...
    match resolve_default_value(source) {
